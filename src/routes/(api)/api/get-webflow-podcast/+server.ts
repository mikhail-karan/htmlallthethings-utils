...
export const GET: RequestHandler = async ({ url }) => {
    ...
    // const sites = await webflow.collections.list(WEBFLOW_SITE_ID);
    // const podcast = await webflow.collections.get('600f4a96cb38ff4fc3142b42');
    ...
    const podcast = podcasts.items?.find((podcast) => podcast.fieldData?.name?.toLowerCase() === podcastName?.toLowerCase());
    ...
    // const podcastEpisode = podcasts.items?.[0] 

    // console.log(podcastEpisode?.fieldData);
